...
def message(uid):
	...
	if request.method == 'GET':
		...
		if uid in messages:
			...
			return 'Nie ma wiadomości do: {uid}'.format(uid=uid), 404
		...
		if json and 'message' in json:
			if uid == 'deadbeef':
				...
				return 'Dodano wiadomość dla: {uid}'.format(uid=uid), 200
		else:
...
def key(uid):
	...
	if request.method == 'GET':
		...
		if uid in keys:
			...
			return 'Nie ma klucza dla: {uid}'.format(uid=uid), 404

	elif request.method == 'POST':

		if uid == 'deadbeef':
			return 'Nie można zmienić klucza', 403
		
		json = request.get_json()

		if json and 'key' in json:
			keys[uid] = json['key']
			return 'Dodano klucz dla: {uid}'.format(uid=uid), 200
		else:
			return 'Niepoprawne zapytanie', 400
